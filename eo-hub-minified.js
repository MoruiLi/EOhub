        ...
        if (_clientContext.isAnonymous == true){
            ...
            await jQuery.ajax(ajaxUrl, {
                ...
                success: function (data){
                    _user["MemberId"] = data.Items.$values[0].Properties.$values[0].Value;
                    _user["FirstName"] = data.Items.$values[0].Properties.$values[1].Value;
                    _user["LastName"] = data.Items.$values[0].Properties.$values[2].Value;
                    _user["Email"] = data.Items.$values[0].Properties.$values[3].Value;
                    _user["MemberType"] = data.Items.$values[0].Properties.$values[4].Value;
                    _user["ChapterId"] = data.Items.$values[0].Properties.$values[5].Value;
                    _user["ChapterName"] = data.Items.$values[0].Properties.$values[6].Value;
                    _user["JoinDate"] = data.Items.$values[0].Properties.$values[7].Value;
		...
		if( (_mbrID != 'undefined' && _mbrID  != '217')){
		 ...
		 var waitForLC = setInterval(function () {
			...
			var showUnavailableStatus = function(show){
				...
				offlineIcon.innerHTML = '<a class="hide-for-small" target="_blank" href="https://forms.monday.com/forms/e8c3fe8add707b272f27ec171e1e4f58?r=use1"><img id="live-chat-notice" width="82" src="https://member.eonetwork.org/PublishingImages/live-chat-offline.png"></a>';
        ...
        jQuery.ajax(JSON.parse(jQuery("#__ClientContext").val()).baseUrl + "api/iqa?queryname=" + iqaSectorLookup + "&parameter=" + s, {
            ...
            success: function(data) {
                ...
                for (i = 0; i < data.TotalCount; i++) {
                    _code = data["Items"]["$values"][i]["Properties"]["$values"][0]["Value"];
                    _desc = data["Items"]["$values"][i]["Properties"]["$values"][1]["Value"];
    ...
    function updateSectors(s) {
        ...
        jQuery.ajax(JSON.parse(jQuery("#__ClientContext").val()).baseUrl + "api/iqa?queryname=$/EORest/GetSectors&parameter=" + s, {
            ...
            success: function(data) {
                var arr = [];
                for (i = 0; i < data.TotalCount; i++) {
                    arr.push(data["Items"]["$values"][i]["Properties"]["$values"][0]["Value"]);
                }
