    ...
    *==============================================================================*/
    async function retrieveUserData() {
        ...
        if (_clientContext.isAnonymous == true){
            ...
            var userData = data.Items[0].Properties;
            for (var propName in userData) {
                if (userData.hasOwnProperty(propName)) {
                    switch (propName) {
                        case "MemberId":
                            _user["MemberId"] = userData[propName].Value;
                            break;
                        case "FirstName":
                            _user["FirstName"] = userData[propName].Value;
                            break;
                        case "LastName":
                            _user["LastName"] = userData[propName].Value;
                            break;
                        case "Email":
                            _user["Email"] = userData[propName].Value;
                            break;
                        case "MemberType":
                            _user["MemberType"] = userData[propName].Value;
                            break;
                        case "ChapterId":
                            _user["ChapterId"] = userData[propName].Value;
                            break;
                        case "ChapterName":
                            _user["ChapterName"] = userData[propName].Value;
                            break;
                        case "JoinDate":
                            _user["JoinDate"] = userData[propName].Value;
                            break;
                        default:
		 ...
		 var waitForLC = setInterval(function () {
			...
			LC_API.on_after_load = function() {
				if (LC_API.agents_are_available()) {
					...
					showUnavailableStatus(true); // showUnavailableStatus(false);
				} else {
					showUnavailableStatus(false); // showUnavailableStatus(true);
				}
			};
